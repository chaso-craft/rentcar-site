...
function renderCarPhotoGallery() {
  const gallery = document.getElementById("carPhotoGallery");
  if (!gallery || !isValidCarType(carType)) return;
  gallery.innerHTML = "";
  const images = getCarImages(carType);
  if (images.length === 0) {
    gallery.appendChild(createCarVisualElement(carType, "car-detail-visual"));
    return;
  }
  images.forEach((item, index) => {
    const img = document.createElement("img");
    img.className = "car-detail-photo";
    img.src = item.dataUrl;
    img.alt = `${getCarLabel(carType)}の写真${index + 1}`;
...
async function initPage() {
  ...
  carTypeTitleEl.textContent = getCarLabel(carType);
  carTypeIntroEl.textContent = getCarCatalogEntry(carType).description || "車種の紹介文は準備中です。";
  renderCarPhotoGallery();
...
ensureDataLoaded()
  ...
  .catch((error) => {
    console.error(error);
    carTypeTitleEl.textContent = "データを読み込めません";
    carTypeIntroEl.textContent = error.message || "しばらくしてから再度お試しください。";
    rentButtonEl.hidden = true;
  });
